...
#define our (*Our)
	...
	*end = c;
	...
	if ((graph->down[graph->children - 1]->options[0] == '[') && (graph->down[graph->children - 1]->options[1] == ']'))
	{
		if (out.y > 1)
		{
	...
	else if ((graph->down[graph->children - 1]->options[0] == '[') || (graph->down[graph->children - 1]->options[1] ==
																	   ']'))
	{
		if (out.y > 1)
		{
